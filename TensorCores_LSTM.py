...
gridDim_x = (batch_size + (WMMA_M * blockDim_x // 32 - 1)) // (WMMA_M * blockDim_x // 32)
gridDim_y = (hidden_units + WMMA_N * blockDim_y - 1) // (WMMA_N * blockDim_y)
mod = SourceModule(
    """
    #include <mma.h>
    #include <cuda_fp16.h>
    ...
    extern "C" __global__ void wmma_example_gates(half *A1, half *B1, float *fa, half *B2, float *ia, half *B3, 
        ...
        int M =""" + str(batch_size) + """;
        int K =""" + str(hidden_units + input_units) + """;
        int N =""" + str(hidden_units) + """;
...
""", arch="sm_70", no_extern_c=True)

bsHddnPlusIn_hddnPlusInHddn = mod.get_function("wmma_example_gates")
...
def lstm_cell(batch_dataset, prev_activation_matrix, prev_cell_matrix, parameters):
    ...
    if IsTraining:
        ...
        bsHddnPlusIn_hddnPlusInHddn(g_gpu1, h_gpu1, i_gpu1, h_gpu2, i_gpu2, h_gpu3, i_gpu3, h_gpu4, i_gpu4,
                                    prev_cell_matrix_gpu, cell_memory_matrix_gpu, activation_matrix_gpu,
                                    block=(blockDim_x, blockDim_y, 1), grid=(gridDim_x, gridDim_y, 1))
    ...
    lstm_activations['fa'] = fa.T
    lstm_activations['ia'] = ia.T
    lstm_activations['oa'] = oa.T
    lstm_activations['ga'] = ga.T
...
print("Batch {0}, TensorCores, {1} hidden".format(batch_size, hidden_units))
embeddings, parameters, J, P, A = train(train_dataset, iters=201, batch_size=batch_sizee)
print("--- %s seconds ---" % (time.time() - start_time))
...
plt.xlabel("x")
plt.ylabel("Accuracy (Promedio en 30 batches)")
plt.title("Accuracy")
plt.show()
