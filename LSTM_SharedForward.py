...
mod = DynamicSourceModule(
    ...
    __global__ void FullForward(float *A1, float *B1, float *fa, float *B2, float *ia,float *B3, float *oa, 
                float *B4, float *ga, float *prev_cell_matrix, float *cell_memory_matrix, float *activation_matrix,
                float *how, float *output_matrix){
        int outARows =""" + str(batch_size) + """; 
        int outAColumns =""" + str(hidden_units) + """;
        int outBRows =""" + str(hidden_units) + """;
        int outBColumns =""" + str(vocab_size) + """;
        int outCRows =""" + str(batch_size) + """;
        int outCColumns =""" + str(vocab_size) + """;
        ...
        __shared__ float sOA1[Tile_size][Tile_size];
        __shared__ float sOB1[Tile_size][Tile_size];

        float CvalueO1 = 0.0;

        sOA1[threadIdx.y][threadIdx.x] = 0.0;
        sOB1[threadIdx.y][threadIdx.x] = 0.0;

        for (int k = 0; k < (((outAColumns)/ Tile_size)); k++){
            if ( (Row < outARows) && (threadIdx.x + (k*Tile_size)) < outAColumns){
                sOA1[threadIdx.y][threadIdx.x] = activation_matrix[(Row*outAColumns) + threadIdx.x + (k*Tile_size)];
            }
            if ( Col < outBColumns && (threadIdx.y + k*Tile_size) < outBRows){
                sOB1[threadIdx.y][threadIdx.x] = how[(threadIdx.y + k*Tile_size)*outBColumns + Col];
            }
            __syncthreads();

            for (int j = 0; j < Tile_size; ++j){
                CvalueO1 += sOA1[threadIdx.y][j] * sOB1[j][threadIdx.x];
            }
            __syncthreads();
        }
        if (Row < outCRows && Col < outCColumns){
            output_matrix[Row*numCColumns + Col] = __expf(Cvalue1);
        }
    }
    """)
...
bsHddnPlusIn_hddnPlusInHddn = mod.get_function("FullForward")
...
# single layer lstm
def FullForward(batch_dataset, prev_activation_matrix, prev_cell_matrix, parameters):
    # INPUT --> HIDDEN
    # parametros
    Tile_size = 2
    how = parameters['how']
    ...
    concat_dataset = np.concatenate((batch_dataset, prev_activation_matrix), axis=1)
    if IsTraining:
        g0 = how.astype(np.float32)
        output_matrix = np.zeros((batch_size, vocab_size)).astype(np.float32)
        ...
        g1_gpu = cuda.mem_alloc(g0.nbytes)
        cuda.memcpy_htod(g1_gpu, g0)
        g_gpu1 = cuda.mem_alloc(g1.nbytes)
        cuda.memcpy_htod(g_gpu1, g1)
        h_gpu1 = cuda.mem_alloc(h1.nbytes)
        cuda.memcpy_htod(h_gpu1, h1)
        ...
        output_matrix_gpu = cuda.mem_alloc(output_matrix.nbytes)
        cuda.memcpy_htod(output_matrix_gpu, output_matrix)

        bsHddnPlusIn_hddnPlusInHddn(g_gpu1, h_gpu1, i_gpu1, h_gpu2, i_gpu2, h_gpu3, i_gpu3, h_gpu4, i_gpu4,
                                    prev_cell_matrix_gpu,
                                    cell_memory_matrix_gpu, activation_matrix_gpu, g1_gpu, output_matrix_gpu, block=(Tile_size, Tile_size, 1),
                                    grid=((i4.shape[1] // Tile_size) + 1, (i4.shape[0] // Tile_size) + 1, 1))
        ...
        cuda.memcpy_dtoh(cell_memory_matrix, cell_memory_matrix_gpu)
        cuda.memcpy_dtoh(activation_matrix, activation_matrix_gpu)
        cuda.memcpy_dtoh(output_matrix, output_matrix_gpu)
    else:
        # forget gate
        fa = np.matmul(concat_dataset, fgw)
        fa = sigmoid(fa)
    ...
    # Guardar valores de activación para el backpropagation
    print(np.allclose(np.exp(np.matmul(activation_matrix, how)), output_matrix, 1e-04, 1e-08))
    lstm_activations = dict()
    lstm_activations['fa'] = fa
    lstm_activations['ia'] = ia
    lstm_activations['oa'] = oa
    ...
    return lstm_activations, cell_memory_matrix, activation_matrix, softmax_FF(output_matrix)
...
def forward_propagation(batches, parameters, embeddings):
    ...
    for i in range(len(batches) - 1):
        ...
        # lstm
        lstm_activations, ct, at, ot = FullForward(batch_dataset, a0, c0, parameters)
...
embeddings, parameters, J, P, A = train(train_dataset, iters=1001, batch_size=batch_sizee)
print("--- %s seconds ---" % (time.time() - start_time))
